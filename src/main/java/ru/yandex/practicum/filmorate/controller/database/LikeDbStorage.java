...
public class LikeDbStorage implements LikeStorage {
    ...
    public void saveLike(Likes likes) {
        jdbcTemplate.update(SAVE_LIKE, likes.getUser().getId(), likes.getFilm().getId());
    }

    @Override
    public void deleteLike(Likes likes) {
        jdbcTemplate.update(DELETE_LIKE, likes.getUser().getId(), likes.getFilm().getId());
    }
    ...
    private Film mapRowToFilm(ResultSet rs, Set<Genre> genres) throws SQLException {
        return new Film(rs.getInt("film_id"), rs.getString("name"),
                ...
                rs.getInt("duration"),
                new RatingMPA(rs.getInt("mpa_id"),
                        rs.getString("title")),
